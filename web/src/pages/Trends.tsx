...
import { cohortInfo, findIndicator, firstYear, isSuicideMeasure, latestYear, SURVEY_YEARS, type Catalog } from '../lib/catalog'
...
import { DEFAULT_INDICATOR, resolveTrends, splitGroups, trendsPath, type TrendsState } from '../lib/routes'
...
function TrendsView({ catalog, state }: { catalog: Catalog; state: TrendsState }) {
  ...
  const title = `${indicator.label}, ${firstYear(indicator)}–${latestYear(indicator)}`
  ...
  return (
    <>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <CohortSwitcher catalog={catalog} cohort={cohort} hrefFor={(c) => trendsPath(c, findIndicator(catalog, c, indicator.id)?.id ?? DEFAULT_INDICATOR)} />
      </div>
        ...
        <div className="rounded-3xl bg-surface p-6 ring-1 ring-line">
          {shard.status === 'loading' ? (
            ...
            <TrendChart
              ...
              series={data.series}
              years={SURVEY_YEARS}
              caption={`Share of U.S. ${info.phrase}${split ? `, by ${split.label.toLowerCase()}` : ''}. Shaded bands show 95% confidence intervals.`}
              annotations={data.annotations}
              notes={data.notes}
              exports={{ csv: toCsv(data.csv), filename: safeFilename(`${cohort}-${indicator.id}-trend${split ? `-by-${split.id}` : ''}`), citation: citation({ title, url: window.location.href }) }}
            />
          )}
        </div>
...
export default function Trends() {
  ...
  const { redirect, state } = resolveTrends(catalog.data, params.cohort, params.indicator, search)
  if (redirect) return <Navigate replace to={redirect} />
  return <TrendsView key={`${state.cohort}/${state.indicator.id}`} catalog={catalog.data} state={state} />
}
